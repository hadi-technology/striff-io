...
export const exampleReviews: ExampleReview[] = [
  {
    ...
    checkHeader: "Striff flagged the inverted plugin boundary before merge, with the first core-to-plugin edge named in the check.",
    scoreboard: [
  ...
  },
  {
    ...
    checkHeader: "Striff flagged both new boundary crossings and the 3-layer skip before merge.",
    scoreboard: [
  ...
  },
  {
    ...
    body: "This PR removes require() calls so bundlers can statically analyze TypeORM. Striff caught the structural side effect: the React Native and Expo drivers now depend directly on src.platform, two boundary crossings with no prior edge in that direction, each skipping a layer.",
    ...
    checkHeader: "Striff flagged both new driver-to-platform crossings and the layer skip on each of them, before merge.",
    scoreboard: [
      { value: "4", label: "findings posted on the live PR", tone: "striff" },
      { value: "2", label: "new driver-to-platform boundary crossings", tone: "other" },
      { value: "3", label: "layer each new edge skips on the way to `src.platform`", tone: "overlap" }
    ],
    hero: {
      stat: "2",
    ...
    highlights: [
      ...
      "Fix the dynamic require in `PlatformTools.load` before it ships in bundled apps."
    ]
  },
  {
    id: "efcore",
    ...
    body: "This PR fixes split-include queries dropping child rows under concurrent inserts. Striff caught the structure: SplitQueryResultCoordinator in Query.Internal now depends on the root Microsoft.EntityFrameworkCore namespace, where the PR's new DbQueryConcurrencyException lives. That direction never existed before, it skips 3 layers, and it leaves the query namespaces one step from a full cycle.",
    ...
    checkHeader: "Striff flagged the new upward edge, the 3-layer skip, and the near-cycle it left behind, before merge.",
    scoreboard: [
      { value: "8", label: "findings posted on the live PR", tone: "striff" },
      { value: "3", label: "layers skipped by the new upward edge", tone: "other" },
      { value: "1", label: "near-cycle left across the query namespaces", tone: "overlap" }
    ],
    hero: {
      stat: "3",
