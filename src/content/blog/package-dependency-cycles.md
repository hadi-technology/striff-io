...
Start with what you already have. Most ecosystems can print a package dependency graph and list the cycles already in it, and if you do nothing else, doing that once a quarter will at least tell you where the loops are. The by-hand version of the per-change check is step 6 of [our architecture review checklist](/blog/architecture-review-checklist).
...
Then write the rule down. If your architecture notes say *"`domain` does not depend on `api`"*, that sentence is checkable as written. Striff reads it out of the document and evaluates it at both revisions of every pull request, so the change that breaks it is told which sentence it broke, in your team's own words, instead of in a tool's.
...
That per-pull-request comparison is what Striff does. It parses both revisions and compares them. **Package cycles** and **Cycle seeds** are two of its fourteen structural checks, reported in the change that introduced the problem, before merge, next to every rule your own docs state about the same code. On the large majority of pull requests it will report clean and list what it looked at, because on the large majority of pull requests nothing happened to the graph. [Install it on a repository](https://github.com/apps/striff-app/installations/new); there is nothing to configure.
