...
Most AI reviewers work prose-first: hand the diff (plus retrieved context) to an LLM and let it write what it notices. The output quality *is* the model's judgment — there's nothing underneath to check against. When it's right, it's genuinely useful. When it's wrong, **it's wrong in fluent, confident English**, and the reviewer has to do the analysis themselves just to find out which one they got.
...
<div class="bp-legend" style="margin-top:0;margin-bottom:0.6rem"><span class="bp-chip" style="--bp-chip-color:#d97706">Prose-first (typical AI reviewer)</span></div>
...
<p class="bp-figure-caption">On our <a href="/blog/architectural-findings-in-oss">head-to-head example PRs</a>, Striff posted <strong>4 to 8 findings per pull request</strong> — not forty. A finding that makes a reviewer shrug costs more trust than a finding we skip.</p>
</div>
...
AI assistants are pushing PR volume up across the industry. Review bots are multiplying in the same feeds. The scarce resource is no longer *analysis* — it's **developer attention and trust**. A reviewer tool only works if engineers still read it in month six.

That's the bet behind the facts-first design: fewer findings, every one traceable to a measured property of your dependency graph, in the PR where the risk appears. It's also why the approach is complementary to the AI reviewers you already run — [on identical PRs, the overlap between Striff and Copilot or Qodo was zero](/blog/architectural-findings-in-oss). They read the implementation. Striff measures the structure.
