...
<p class="bp-figure-caption">Step ② is the only neural part: a model read a bullet point and proposed a rule. It survived grounding because both names resolve to real types in the parsed code. Steps ③ and ④ are lookups in two public revisions, and the sentence at the bottom can only be assembled from them. The verdict is symbolic; only the word order is neural.</p>
</div>
