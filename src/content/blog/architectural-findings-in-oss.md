...
That number is easy to hide and easy to fake, so here it is measured, on a window of thirty real pull requests from eleven public repositories, analysed with the same pipeline that runs on a live one. Twenty-one of the thirty produced no structural finding at all. This post is every documented rule the window evaluated, every structural finding from the other nine, the two findings we think were weak and why, and the questions the system declined to answer.
...
## The rules the repositories wrote themselves

Start with the checks that are not ours. The most persuasive rules in any review are the repository's own, because a maintainer does not argue with a sentence they wrote.
...
<div class="bp-bars">
<div class="bp-bar-row"><span class="bp-bar-label">Held</span><div class="bp-bar-track"><div class="bp-bar bp-bar--mint" style="width:100%"></div></div><span class="bp-bar-value">39</span></div>
<div class="bp-bar-row"><span class="bp-bar-label">Couldn't check</span><div class="bp-bar-track"><div class="bp-bar" style="width:15%"></div></div><span class="bp-bar-value">6</span></div>
<div class="bp-bar-row"><span class="bp-bar-label">Pre-existing</span><div class="bp-bar-track"><div class="bp-bar bp-bar--amber" style="width:10%"></div></div><span class="bp-bar-value">4</span></div>
<div class="bp-bar-row"><span class="bp-bar-label">Violated by the change</span><div class="bp-bar-track"><div class="bp-bar bp-bar--danger" style="width:0%"></div></div><span class="bp-bar-value">0</span></div>
</div>
<p class="bp-figure-caption">Zero, in this window. A documented rule broken by the pull request under review is a rare event, and a tool that produced them at a comfortable rate would be producing them wrongly. The other three rows are the ones that do the work. A week later, a larger scan of 74 pull requests produced 1,912 rule evaluations and exactly one violation, and <a href="/blog/design-docs-are-enforceable-now">it is a good one</a>.</p>
</div>

### A rule that held is the product working
...
<div class="bp-callout"><strong>"Could not analyse" must never persist as "analysed, found nothing."</strong> A source parser sees less than a compiler: no generated members, no annotations, no string literals, no reflection. Any tool that collapses those blind spots into a green tick is lying at exactly the moment you are trusting it most. Every rule reports one of five outcomes, and one of the five is “couldn't check”.</div>
...
## What has to be true before something is a structural finding

The documented rules come from the repository. The structural checks come from us, which is why each one has to clear a single question: **does a reviewer already know this from the diff or from the diagram?**
...
<div class="bp-figure" data-reveal>
<p class="bp-figure-title">What each check is looking for</p>
...
<tbody>
<tr><td>Documented rules</td><td>a change making false something this repository's own docs say about the code</td><td>the sentence is in a different file, usually one nobody opened</td></tr>
...
<tr><td>Tests for changed contracts</td><td>a component's public surface changing while the test that already covered it is left untouched</td><td>the test is in another file, and “already covered it” is a fact about the repository</td></tr>
...
<tr><td>Class-level coupling knots</td><td>two classes in different packages now referencing each other where only one direction existed before</td><td>the existing direction is in a file the change never touched</td></tr>
...
<tr><td>Encapsulation</td><td>a component's internals being opened up: its share of private members dropping sharply</td><td>the diff shows a modifier; the share of the class it exposes is not in it</td></tr>
<tr><td>Hub formation</td><td>an existing component with fewer than five dependents acquiring ten or more in one change</td><td>the new dependents are spread across files, and the count before is not in any of them</td></tr>
<tr><td>Type relocations</td><td>a type moved between packages, reported as context rather than a defect</td><td>a move reads as one file deleted and another added</td></tr>
</tbody>
</table>
</div>
<p class="bp-figure-caption">The first row is the repository's; the other fourteen are ours. Every row shares one property: answering it means holding both revisions of the code at once, and the first also means holding the documents. That is the entire scope of the product, and the reason the check is silent so often: most pull requests neither move the graph nor contradict the docs.</p>
</div>

## All thirteen structural findings
...
- **Where do its claims come from?** Every claim above is a sentence quoted verbatim from a file in the repository and checked against two parsed revisions of it, or a property of those two revisions. Every pull request linked here is public. The verdicts are checkable, and we would rather you checked them.
