...
<p class="bp-figure-caption">Roughly two percent accuse: 97 rules broken by the change under review, 53 already broken before it. Seventy-seven percent simply held, which is what a correct document looks like. The third row down is the one we would defend hardest: 1,487 rules the system declined to answer, and said so. A parser sees less than a compiler, and &ldquo;could not check&rdquo; turning into &ldquo;checked, found nothing&rdquo; is the failure this design exists to prevent. Accusations also concentrate: a single repository accounts for half of them, so the per-repository rate is far lower than the per-verdict rate suggests.</p>
</div>
